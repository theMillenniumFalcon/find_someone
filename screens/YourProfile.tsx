...
import { ScrollView, View, ImageBackground, TouchableOpacity, Platform, Alert, RefreshControl, Keyboard } from "react-native"
...
import { UserInterestAutocomplete, YourProfileResource, UserMiscInfoEnum, UserInterest, UnitsEnum, UserDto } from "../types/types"
...
const YourProfile = ({ route, navigation }: any) => {

    const { colors } = useTheme()
    const [refreshing, setRefreshing] = useState(false)
    const [data, setData] = useState<YourProfileResource>()
    ...
    const [interests, setInterests] = useState(Array<UserInterest>)
    ...
    const [minAgeText, setMinAgeText] = useState(MIN_AGE)
    const [maxAgeText, setMaxAgeText] = useState(MAX_AGE)
    const dropdownController = useRef() as any
    const [units, setUnits] = useState(UnitsEnum.SI)

    const [isLegal, setIsLegal] = useState(false)
    const [miscInfoDrugsTobacco, setMiscInfoDrugsTobacco] = useState(false)
    const [miscInfoDrugsAlcohol, setMiscInfoDrugsAlcohol] = useState(false)
    const [miscInfoDrugsCannabis, setMiscInfoDrugsCannabis] = useState(false)
    const [miscInfoDrugsOther, setMiscInfoDrugsOther] = useState(false)
    const [miscInfoKidsNo, setMiscInfoKidsNo] = useState(false)
    const [miscInfoKidsYes, setMiscInfoKidsYes] = useState(false)
    const [miscInfoRelationShipSingle, setMiscInfoRelationShipSingle] = useState(false)
    const [miscInfoRelationShipTaken, setMiscInfoRelationShipTaken] = useState(false)
    const [miscInfoRelationShipOpen, setMiscInfoRelationShipOpen] = useState(false)
    const [miscInfoRelationShipOther, setMiscInfoRelationShipOther] = useState(false)
    ...
    const debounceDescriptionHandler = useCallback(debounce(updateDescription, 1500), [])
    ...
    async function load() {
        ...
        let gendersData = data.user.preferedGenders.map((item: { text: any }) => item.text)
        ...
        let miscInfoData = data.user.miscInfos.map((item: { value: any }) => item.value)
